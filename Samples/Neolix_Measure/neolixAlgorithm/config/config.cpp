...
DLLTEST_API bool measureVol(depthData depth_, vol &v, int method)
{
...
DLLTEST_API void setParameter(IN__ double backgroundParameter[], int method)
{
    if(0 == method)
    {
...
DLLTEST_API bool backgroundReconstruction(depthData depth_, double parameter[], int &n, int method)
{
    void* data = malloc(depth_.width*depth_.height*sizeof(short));
    memcpy(data,(void*)depth_.data,depth_.width*depth_.height*sizeof(short));
    cv::Mat d = cv::Mat(depth_.height,depth_.width,CV_16UC1,data);
...
DLLTEST_API bool setArea(rect safeZone, rect planeZone, double internalCoefficient[], int n)
{
    if(n < 4) return false;
